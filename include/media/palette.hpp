...
#   endif
    ...
    private:
        static __forceinline constexpr uint16_t to_ste(uint8_t c, uint8_t shift) {
            static constexpr uint8_t STE_TO_SEQ[16] = { 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15 };
            const int i = c >> 4;
            __assume_count(i, 16);
            return STE_TO_SEQ[i] << shift;
        }
        static __forceinline constexpr uint8_t from_ste(uint16_t c, uint8_t shift) {
            static constexpr uint8_t STE_FROM_SEQ[16] = { 0x00, 0x22, 0x44, 0x66, 0x88, 0xaa, 0xcc, 0xee,
